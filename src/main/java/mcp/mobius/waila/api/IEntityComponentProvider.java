...
public interface IEntityComponentProvider
{
	...
	/**
	 * Callback used to set an item to display alongside the entity name in the
	 * tooltip, similar to how blocks are treated. Will only be called if the
	 * implementing class is registered via {@link}
	 *
	 * @param accessor
	 *            Contains most of the relevant information about the current
	 *            environment.
	 ...
	 * Body, Tail).</br>
	 * Will only be called if the implementing class is registered via
	 * {@link IRegistrar#registerComponentProvider(IEntityComponentProvider, TooltipPosition, Class)}.</br>
