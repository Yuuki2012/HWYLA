...
public class HUDHandlerEntities implements IEntityComponentProvider
{
	...
	{
		((ITaggableList<Identifier, TextComponent>) tooltip).setTag(HUDHandlerBlocks.OBJECT_NAME_TAG, new TextComponent(String.format(Waila.CONFIG.get().getFormatting().getEntityName(), accessor.getEntity().getDisplayName().getFormattedText())));

		if (config.get(PluginCore.CONFIG_SHOW_REGISTRY))
			((ITaggableList<Identifier, TextComponent>) tooltip).setTag(HUDHandlerBlocks.REGISTRY_NAME_TAG, (TextComponent) new TextComponent(Registry.ENTITY_TYPE.getId(accessor.getEntity().getType()).toString()).setStyle(new Style().setColor(ChatFormat.GRAY)));
	}
	...
	public void appendBody(List<TextComponent> tooltip, IEntityAccessor accessor, IPluginConfig config)
	{
		if (config.get(PluginCore.CONFIG_SHOW_ENTITY_HEALTH) && accessor.getEntity() instanceof LivingEntity)
		{
			...
			if (living.getHealthMaximum() > Waila.CONFIG.get().getGeneral().getMaxHealthForRender())
				tooltip.add((TextComponent) new TranslatableComponent("tooltip.waila.health", String.format("%.2f", health), String.format("%.2f", maxHealth)).append(""));
			else
			{
	...
	{
		((ITaggableList<Identifier, TextComponent>) tooltip).setTag(HUDHandlerBlocks.MOD_NAME_TAG, new TextComponent(String.format(Waila.CONFIG.get().getFormatting().getModName(), ModIdentification.getModInfo(accessor.getEntity()).getName())));
	}
}