...
public class ModIdentification
{
	...
	{
		return CONTAINER_CACHE.computeIfAbsent(namespace, s -> FabricLoader.getInstance().getAllMods().stream().map(ModContainer::getMetadata).filter(m -> m.getId().equals(s)).findFirst().orElse(new DummyModMetadata(s, s)));
	}
